...
namespace UnrealConsole {
	...
	void UnrealConsole::ProcessCommand(wstring input) {
		...
		case Hashes::connect:
			...
			Log("You are already connected!", LogType::System); // TODO double check ap connection?
			break;
		...
		case Hashes::help:
			Logger::PrintToConsole(L"/" + input);
			Log("TODO", LogType::System); // TODO make a help string
			break;
		case Hashes::popups: {
			Logger::PrintToConsole(L"/" + input);
		...
		default:
			...
			Log(L"Known commands: help, popups, spawn", LogType::System);
			break;
		}
	}
	...
	void UnrealConsole::ProcessCommand(const TCHAR* new_command) {
		...
		// TODO can we just get rid of this whole function?
